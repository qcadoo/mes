...
public class OrderStateChangingService {
    ...
    public List<ChangeOrderStateError> validationPending(final Entity entity) {
    ...
    public List<ChangeOrderStateError> validationAccepted(final Entity entity) {
        checkArgument(entity != null, "entity is null");
        List<String> references = Arrays.asList("product", "plannedQuantity", "dateTo", "dateFrom", "technology");
        return checkValidation(references, entity);
    }

    public List<ChangeOrderStateError> validationInProgress(final Entity entity) {
        checkArgument(entity != null, "entity is null");
        return validationAccepted(entity);
    }

    public List<ChangeOrderStateError> validationCompleted(final Entity entity) {
        checkArgument(entity != null, "entity is null");
        List<String> references = Arrays.asList("product", "plannedQuantity", "dateTo", "dateFrom", "technology", "doneQuantity");
        return checkValidation(references, entity);
    }

    private List<ChangeOrderStateError> checkValidation(final List<String> references, final Entity entity) {
        checkArgument(entity != null, "entity is null");
        List<ChangeOrderStateError> errors = new ArrayList<ChangeOrderStateError>();
        for (String reference : references)
            if (entity.getField(reference) == null) {
                ChangeOrderStateError error = new ChangeOrderStateError();
