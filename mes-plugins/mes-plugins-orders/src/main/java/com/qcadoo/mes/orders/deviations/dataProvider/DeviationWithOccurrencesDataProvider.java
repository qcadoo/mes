...
public class DeviationWithOccurrencesDataProvider {
    ...
    private static final String PARTIAL_COUNT_SUB_QUERY_TPL = "(select count(*) from #orders_${MODEL_NAME} where ${REASON_TYPE_FIELD_NAME} = di.name and date >= :"
            + L_DATE_FROM + " AND date <= :" + L_DATE_TO + ")";
        ...
        Collection<String> subQueries = Collections2.transform(Arrays.asList(DeviationType.values()),
                new Function<DeviationType, String>() {
                    ...
                    public String apply(final DeviationType deviationType) {
                        StrSubstitutor substitutor = new StrSubstitutor(ImmutableMap.of("MODEL_NAME",
                                deviationType.getReasonModelName(), "REASON_TYPE_FIELD_NAME",
                                deviationType.getReasonTypeInReasonModelFieldName()), "${", "}");
