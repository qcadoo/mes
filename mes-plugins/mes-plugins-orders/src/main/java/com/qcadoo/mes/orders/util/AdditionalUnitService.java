...
public class AdditionalUnitService {
    ...
    public BigDecimal getQuantityAfterConversion(final Entity order, String givenUnit, BigDecimal quantity, String baseUnit) {
        ...
        if (!baseUnit.equals(givenUnit)) {
            if (unitConversions.isDefinedFor(givenUnit)) {
                return unitConversions.convertTo(quantity, givenUnit, BigDecimal.ROUND_FLOOR);
            }
        }
    ...
    public void setQuantityFieldForAdditionalUnit(final ViewDefinitionState view, final Entity order) {
        ...
        if (!isValidDecimalField(view, Lists.newArrayList(OrderFields.PLANNED_QUANTITY), true)) {
            return;
        }
    ...
    public void setQuantityForUnit(final ViewDefinitionState view, final Entity order) {
        ...
        if (!isValidDecimalField(view, Lists.newArrayList(OrderFields.PLANNED_QUANTITY_FOR_ADDITIONAL_UNIT), true)) {
            return;
        }
    ...
    public void setAdditionalUnitField(final ViewDefinitionState state) {
        FormComponent orderForm = (FormComponent) state.getComponentByReference(QcadooViewConstants.L_FORM);
    ...
    public boolean isValidDecimalFieldWithoutMsg(final ViewDefinitionState view, final List<String> fields) {
        return isValidDecimalField(view, fields, false);
    }

    private boolean isValidDecimalField(final ViewDefinitionState view, final List<String> fields, final boolean addMessage) {
