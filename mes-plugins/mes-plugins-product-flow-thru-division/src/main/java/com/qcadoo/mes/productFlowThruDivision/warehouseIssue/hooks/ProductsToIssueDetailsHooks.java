...
public class ProductsToIssueDetailsHooks {
    ...
    public void onBeforeRender(final ViewDefinitionState view) {
        ...
        if (view.isViewAfterRedirect() && helper.getHasManyField("issues").isEmpty()) {
            ...
            for (Long id : ids) {
                ...
                createdIssues.add(createIssue(issueDD, productToIssue));
            }
    ...
    private List<Entity> sortIssuesBasedOnFilter(ViewDefinitionState view, List<Entity> createdIssues) {
        try {
            sortEntries: {
    ...
    private void fillLocationQuantity(List<Entity> createdIssues, Map<Long, Map<Long, BigDecimal>> stockForWarehousesFrom,
            Map<Long, Map<Long, BigDecimal>> stockForWarehousesTo) {
        for (Entity issue : createdIssues) {
            BigDecimal quantityFrom = stockForWarehousesFrom.get(
                    issue.getBelongsToField(IssueFields.WAREHOUSE_ISSUE).getBelongsToField(WarehouseIssueFields.PLACE_OF_ISSUE)
                            .getId()).get(issue.getBelongsToField(IssueFields.PRODUCT).getId());
            BigDecimal quantityTo = stockForWarehousesTo.get(issue.getBelongsToField(IssueFields.LOCATION).getId()).get(
                    issue.getBelongsToField(IssueFields.PRODUCT).getId());
            issue.setField(IssueFields.LOCATIONS_QUANTITY, quantityFrom);
            issue.setField(IssueFields.LOCATION_TO_QUANTITY, quantityTo);
        }
    }

    private Map<Long, Map<Long, BigDecimal>> getStockForWarehouses(Multimap<Entity, Entity> locationProductMultimap) {
        ...
        for (Entity warehouse : locationProductMultimap.keySet()) {
            ...
            Map<Long, BigDecimal> stockMap = materialFlowResourcesService
                    .getQuantitiesForProductsAndLocation(products, warehouse);
    ...
    public void fillQuantitiesInAdditionalUnit(final ViewDefinitionState view) {
        ...
        for (FormComponent form : adl.getFormComponents()) {
            ...
            Either<Exception, Optional<BigDecimal>> maybeQuantity = BigDecimalUtils.tryParse(quantityField.getFieldValue()
    ...
    private Entity createIssue(final DataDefinition issueDD, Entity productToIssue) {
        ...
        if (warehouseIssueParameterService.issueForOrder()) {
            ...
            if (plannedQuantity != null && demandQuantity != null) {
                quantityPerUnit = numberService.setScaleWithDefaultMathContext(demandQuantity.divide(plannedQuantity, numberService.getMathContext()));
            }
        }
        Entity product = productToIssue.getBelongsToField(ProductsToIssueFields.PRODUCT);
