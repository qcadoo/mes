...
public final class WorkPlanService {
    ...
    public void generateWorkPlan(final ViewDefinitionState viewDefinitionState, final ComponentState state, final String[] args) {
        ...
        } else if (state.getFieldValue() instanceof Long) {
    ...
    public void printWorkPlanForOrder(final ViewDefinitionState viewDefinitionState, final ComponentState state,
        ...
        if (state.getFieldValue() instanceof Long) {
            ...
            if (order == null) {
                ...
                LOG.debug(">>>>>>>>>>>>>>>> CREATING A NEW WORK PLAN");
                Entity workPlan = createNewWorkPlan(order);

                try {
                    LOG.debug(">>>>>>>>>>>>>>>>> GENERATING A NEW WORK PLAN DOCUMENT");
                    generateWorkPlanDocuments(state, workPlan);
                    LOG.debug(">>>>>>>>>>>>>>>>> GENERATED A NEW WORK PLAN DOCUMENT");
                } catch (IOException e) {
                    throw new IllegalStateException(e.getMessage(), e);
                } catch (DocumentException e) {
                    throw new IllegalStateException(e.getMessage(), e);
    ...
    private Entity createNewWorkPlan(Entity order) {
        ...
        // Entity singleOrder = new DefaultEntity("products", "orders");
        // singleOrder.setField("order", order);
