...
public final class ProductService {
    ...
    public void printMaterialRequirementForOrder(final ViewDefinitionState viewDefinitionState, final ComponentState state,
            final String[] args) {
        ...
        if (state.getFieldValue() instanceof Long) {
            ...
            // requirement.setFileName("test");
            // requirement.setGenerated(true);
            // requirement.setWorker("worker");
            ...
            if (order == null) {
                ...
                Entity workPlan = createNewWorkPlan(order);
    ...
    private Entity createNewWorkPlan(Entity order) {
        Entity workPlan = new DefaultEntity("base", "autoGeneratedPlan");

        workPlan.setField("name", "autoGeneratedPlan");
        workPlan.setField("generated", false);
        workPlan.setField("worker", securityService.getCurrentUser().getUserName());
        workPlan.setField("date", new Date());
        // workPlan.setField("orders", new EntityList(dataDefinition, joinFieldName, parentId));

        // ProductsWorkPlanComponent component = new ProductsWorkPlanComponent();
        // component.setOrder((ProductsOrder) order);
        // component.setWorkPlan(workPlan);
        // ArrayList<ProductsWorkPlanComponent> components = new ArrayList<ProductsWorkPlanComponent>();
        // components.add(component);
        // workPlan.setOrders(components);
        return workPlan;
    }
