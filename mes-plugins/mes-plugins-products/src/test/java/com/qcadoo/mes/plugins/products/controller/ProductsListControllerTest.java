...
public class ProductsListControllerTest extends TestCase {
	...
	public void testProductsList() {
		...
		assertEquals(GridDefinition.class,
				modelAndView.getModel().get("gridDefinition").getClass());
		assertEquals(dds.get("product").getGrids().get(0), modelAndView
				.getModel().get("gridDefinition"));
		assertEquals(
				dds.get("product").getGrids().get(0).getColumns(),
				((GridDefinition) modelAndView.getModel().get("gridDefinition"))
						.getColumns());
	...
	private void testListData(int maxResults, int firstResult) {
		List<Entity> entities = controller.getListData("" + maxResults,
	...
	public void testDeleteData() {
		...
		ProductsListController controller = new ProductsListController(null,
				dasMock);
