...
QCD.resourceHistory = (function() {
    let localStorageKey = 'qcadoo-materialFlowResources-resourceHistory';
    let grid;
    let dataView;
    let options = {
    ...
    function correctionFormatter(row, cell, value, columnDef, dataContext) {
      if (!value) return "";
      return '<a href="main.html#page/materialFlowResources/resourceCorrectionDetails.html?context=%7B%22form.id%22%3A%22' + dataContext['correctionId'] + '%22%2C%22form.undefined%22%3Anull%7D" class="slick-blue-link" target="_blank">' + value + '</a>';
    }

    function repackingFormatter(row, cell, value, columnDef, dataContext) {
      if (!value) return "";
      return '<a href="main.html#page/materialFlowResources/repackingDetails.html?context=%7B%22form.id%22%3A%22' + dataContext['repackingId'] + '%22%2C%22form.undefined%22%3Anull%7D" class="slick-blue-link" target="_blank">' + value + '</a>';
    }
    ...
    function init() {
        let params = {
            number: document.getElementById("window.mainTab.form.gridLayout.number_input").value
        };

        QCD.components.elements.utils.LoadingIndicator.blockElement($('body'));

        $.get("/rest/resourceHistory/validate", params, function(message) {
            if (message) {
                ...
                $.get("/rest/resourceHistory/columns", function(columns) {
                    $('#resourceHistoryGrid').height($('#window_windowContent').height() - 80);
                    $('#resourceHistoryGrid').width($('#window_windowContent').width() - 20);
                    ...
                    for (let i = 0; i < columns.length; i++) {
                       ...
                       if (columns[i].id === 'correctionNumber') {
                            columns[i].formatter = correctionFormatter;
                       }
                       if (columns[i].id === 'repackingNumber') {
                            columns[i].formatter = repackingFormatter;
                       }
                    }

                    let dataView = new Slick.Data.DataView();

                    grid = new Slick.Grid("#resourceHistoryGrid", dataView, columns, options);
    ...
    function exportToCsv() {
        let params = {
            dateFrom: document.getElementById("window.mainTab.form.gridLayout.number_input").value,
            dateTo: null,
        ...
        $.ajax({
            url: "/rest/resourceHistory/exportToCsv",
            type: 'POST',
            dataType: 'json',
