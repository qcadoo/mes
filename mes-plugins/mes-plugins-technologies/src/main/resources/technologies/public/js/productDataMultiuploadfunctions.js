			...
			.fileupload(
					{
						...
						submit : function(e, data) {
							...
							.getComponentByReferenceName(
								"productDataMultiUploadLocale")
									.getValue().content.value;
							var productDataId = window.mainController
								.getComponentByReferenceName(
									"productDataIdForMultiUpload")
										.getValue();
							var productDataIdValue = productDataId.content;
							if(!productDataIdValue.value || 0 === productDataIdValue.value){
							    $.each(data.files, function (index, file) {
									if(locale === "pl_PL" || locale === "pl"){
							    	showMessage("failure",
											"Zamówienie niezapisane",
											"Pominięto wgranie pliku: "
													+ file.name);
									} else {
								    	showMessage("failure",
												"productData not saved",
												"Omitted file upload: "
														+ file.name);
									}
									});
					...
					function(e, data) {
						...
						var productDataIdValue = productDataId.content;
						data.formData = {
								productDataId : productDataIdValue.value
						};
