...
import com.qcadoo.mes.basic.importing.dtos.ImportError;
    ...
    private void propagateErrors(final ImportStatus importStatus, final CellBinderRegistry cellBinderRegistry,
        ...
        importStatus.getErrors().stream().sorted(comparator).forEach(importError -> {
            ...
            if (StringUtils.isNotEmpty(importError.getFieldName())) {
                fieldName = translationService.translate(
                        createFieldNameKey(pluginIdentifier, modelName, importError.getFieldName()),
            ...
            logService.add(LogService.Builder.error(logType, action).withMessage(message).withItem1(fileName)
                    .withItem2(relatedLine).withItem3(fieldName));
        });
    }
