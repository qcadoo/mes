...
@Component
public class ShiftCellParser implements CellParser {
    ...
    public void parse(final String cellValue, final String dependentCellValue, final CellErrorsAccessor errorsAccessor,
        ...
        Entity shift = getShiftByName(cellValue);

        if (Objects.isNull(shift)) {
            errorsAccessor.addError(L_QCADOO_VIEW_VALIDATE_FIELD_ERROR_LOOKUP_CODE_NOT_FOUND);
        } else {
            valueConsumer.accept(shift);
        }
    }

    private Entity getShiftByName(final String name) {
        return getShiftDD().find().add(SearchRestrictions.eq(ShiftFields.NAME, name)).setMaxResults(1).uniqueResult();
    }

    private DataDefinition getShiftDD() {
