...
@Component
public class StaffCellParser implements CellParser {
    ...
    public void parse(final String cellValue, final String dependentCellValue, final CellErrorsAccessor errorsAccessor,
        ...
        Entity staff = getStaffByNumber(cellValue);

        if (Objects.isNull(staff)) {
            errorsAccessor.addError(L_QCADOO_VIEW_VALIDATE_FIELD_ERROR_LOOKUP_CODE_NOT_FOUND);
        } else {
            valueConsumer.accept(staff);
        }
    }

    private Entity getStaffByNumber(final String number) {
        return getStaffDD().find().add(SearchRestrictions.eq(StaffFields.NUMBER, number)).setMaxResults(1).uniqueResult();
    }
