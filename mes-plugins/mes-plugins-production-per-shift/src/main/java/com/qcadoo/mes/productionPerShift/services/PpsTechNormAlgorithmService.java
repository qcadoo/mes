...
@Service public class PpsTechNormAlgorithmService {

    @Autowired private BasicProductionCountingService basicProductionCountingService;
    ...
    @Autowired private PpsTimeHelper ppsTimeHelper;

    @Autowired private WorkersOnLineService workersOnLineService;
    ...
    public void generateProgressForDays(ProgressForDaysContainer progressForDaysContainer, Entity productionPerShift) {
        ...
        plannedQuantity = calculateRegisteredQuantity(progressForDaysContainer, order, operationComponent, plannedQuantity);
        
        List<Shift> shifts = extractShiftsFormOrder(order);
        if (shifts.isEmpty()) {
            progressForDaysContainer.addError(new ErrorMessage("Nie ma zmian przypisanych do lini", false));
            throw new IllegalStateException("No shifts assigned to production lie");
        }
    ...
    private ShiftEfficiencyCalculationHolder calculateShiftEfficiency(ProgressForDaysContainer progressForDaysContainer,
        ...
        int workersOnLine = workersOnLineService
                .getWorkersOnLine(order.getBelongsToField(OrderFields.PRODUCTION_LINE), shift.getEntity(), range.getFrom());
        if (workersOnLine == 0) {
            progressForDaysContainer.addError(new ErrorMessage("Nie ma przydziału do zmian od ", false));
            throw new IllegalStateException("No aasigment for shift");
        }
    ...
    private void calculateEfficiencyTime(ShiftEfficiencyCalculationHolder calculationHolder, BigDecimal shiftEfficiency,
    ...
    private BigDecimal calculateEfficiencyForRange(BigDecimal scaledNorm, int workersOnLine, long minuets) {
    ...
    private BigDecimal getStandardPerformanceNorm(ProgressForDaysContainer progressForDaysContainer, Entity order) {
        BigDecimal norm = order.getBelongsToField(OrderFields.TECHNOLOGY).getDecimalField("standardPerformanceTechnology");
        if (norm == null) {
            progressForDaysContainer.addError(new ErrorMessage("Nie ma normy w tyechnologii", false));
            throw new IllegalStateException("No standard performance norm in technology");
        }
        return norm.setScale(0, RoundingMode.HALF_UP);
    }
    ...
    private List<Shift> extractShiftsFormOrder(final Entity order) {
        ...
        return FluentIterable.from(entityShifts).transform(new Function<Entity, Shift>() {
            ...
            @Override public Shift apply(final Entity shiftEntity) {
    ...
    private BigDecimal calculateRegisteredQuantity(final ProgressForDaysContainer progressForDaysContainer, final Entity order,
        ...
        if (operationComponent != null) {
            dailyProgressesWithTrackingRecords = dailyProgressService
                    .getDailyProgressesWithTrackingRecords(order, operationComponent);
            for (Entity trackingRecord : dailyProgressesWithTrackingRecords.values()) {
                alreadyRegisteredQuantity = alreadyRegisteredQuantity
                        .add(trackingRecord.getDecimalField(DailyProgressFields.QUANTITY));
            }
