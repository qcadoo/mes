...
public class OrderCriteriaModifier {
    ...
    public void orderFilterForProductionBalance(final SearchCriteriaBuilder scb) {
        final List<String> allowStatesList = Lists.newArrayList(IN_PROGRESS.getStringValue(), COMPLETED.getStringValue(),
                DECLINED.getStringValue(), INTERRUPTED.getStringValue(), ABANDONED.getStringValue());
        scb.add(SearchRestrictions.and(SearchRestrictions.in(STATE, allowStatesList),
                SearchRestrictions.ne(TYPE_OF_PRODUCTION_RECORDING, TypeOfProductionRecording.BASIC.getStringValue())))
                .createCriteria("productionRecords", "productionRecords")
