...
class ProductionBalanceRepository {
    ...
    List<ProducedQuantities> getProducedQuantities(final List<Long> ordersIds) {
        ...
        query.append("COALESCE(SUM(topoc.usedquantity),0) AS producedQuantity, COALESCE(SUM(topoc.wastesquantity),0) AS wastesQuantity, 0 as producedWastes, ");
        query.append("plannedQuantity - COALESCE(SUM(topoc.usedquantity),0) AS deviation, product.unit AS productUnit ");
        query.append("from orders_order o ");
        ...
        query.append("group by orderNumber, productNumber, productName, plannedQuantity, productUnit");

        Map<String, Object> params = Maps.newHashMap();
        params.put("ordersIds", ordersIds);

        return jdbcTemplate.query(query.toString(), params, new BeanPropertyRowMapper<>(ProducedQuantities.class));
    }
