...
CREATE OR REPLACE VIEW masterorders_masterorderposition_oneproduct AS SELECT ( SELECT COALESCE(max(masterorders_masterorderproduct.id), 0::integer) FROM masterorders_masterorderproduct ) + row_number() OVER () AS id, masterorderdefinition.number AS masterorderdefinitionnumber, masterorder.id::integer AS masterorderid, masterorder.product_id::integer AS productid, masterorderproduct.id::integer AS masterorderproductid, masterorder.masterordertype, masterorder.name, masterorder.number, masterorder.deadline, masterorder.masterorderstate AS masterorderstatus, masterorder.masterorderpositionstatus, COALESCE(masterorder.masterorderquantity, 0::numeric) AS masterorderquantity, COALESCE((SELECT sum(orders.plannedquantity) AS sum), 0::numeric) AS cumulatedmasterorderquantity, COALESCE((SELECT sum(orders.donequantity) AS sum), 0::numeric) AS producedorderquantity, CASE WHEN (COALESCE(masterorder.masterorderquantity, 0::numeric) - COALESCE((SELECT sum(orders.donequantity) AS sum), 0::numeric)) > 0::numeric THEN COALESCE(masterorder.masterorderquantity, 0::numeric) - COALESCE((SELECT sum(orders.donequantity) AS sum), 0::numeric) ELSE 0::numeric END AS lefttorelease, masterorder.comments, product.number AS productnumber, product.name AS productname, product.unit, technology.number AS technologyname, company.name AS companyname, masterorder.active, companypayer.name AS companypayer, NULL::character varying(255) AS assortmentName FROM masterorders_masterorder masterorder LEFT JOIN masterorders_masterorderdefinition masterorderdefinition ON masterorderdefinition.id = masterorder.masterorderdefinition_id LEFT JOIN masterorders_masterorderproduct masterorderproduct ON masterorderproduct.masterorder_id = masterorder.id LEFT JOIN basic_product product ON product.id = masterorder.product_id LEFT JOIN basic_company company ON company.id = masterorder.company_id LEFT JOIN technologies_technology technology ON technology.id = masterorder.technology_id LEFT JOIN orders_order orders ON orders.masterorder_id = masterorder.id AND orders.product_id = masterorder.product_id LEFT JOIN basic_company companypayer ON companypayer.id = masterorder.companypayer_id WHERE masterorder.masterordertype::text = '02oneProduct'::text GROUP BY masterorderdefinition.number, masterorder.id, masterorder.product_id, masterorderproduct.id, masterorder.masterordertype, masterorder.name, masterorder.deadline, masterorder.masterorderstate, masterorder.masterorderpositionstatus, masterorder.comments, product.number, product.name, product.unit, technology.number, company.name, masterorder.active, companypayer.name;

CREATE OR REPLACE VIEW masterorders_masterorderposition_manyproducts AS SELECT COALESCE(masterorderproduct.id, 0::integer) AS id, masterorderdefinition.number AS masterorderdefinitionnumber, masterorder.id::integer AS masterorderid, masterorderproduct.product_id::integer AS productid, masterorderproduct.id::integer AS masterorderproductid, masterorder.masterordertype, masterorder.name, masterorder.number, masterorder.deadline, masterorder.masterorderstate AS masterorderstatus, masterorderproduct.masterorderpositionstatus, COALESCE(masterorderproduct.masterorderquantity, 0::numeric) AS masterorderquantity, COALESCE((SELECT SUM(orders.plannedquantity)), 0::numeric) AS cumulatedmasterorderquantity, COALESCE((SELECT SUM(orders.donequantity)), 0::numeric) AS producedorderquantity, CASE WHEN (COALESCE(masterorderproduct.masterorderquantity, 0::numeric) - COALESCE((SELECT SUM(orders.donequantity)), 0::numeric)) > 0 THEN (COALESCE(masterorderproduct.masterorderquantity, 0::numeric) - COALESCE((SELECT SUM(orders.donequantity)), 0::numeric)) ELSE 0::numeric END AS lefttorelease, masterorderproduct.comments, product.number AS productnumber, product.name AS productname, product.unit, technology.number AS technologyname, company.name AS companyname, masterorder.active, companypayer.name AS companypayer, assortment.name AS assortmentName FROM masterorders_masterorder masterorder LEFT JOIN masterorders_masterorderdefinition masterorderdefinition ON masterorderdefinition.id = masterorder.masterorderdefinition_id LEFT JOIN masterorders_masterorderproduct masterorderproduct ON masterorderproduct.masterorder_id = masterorder.id LEFT JOIN basic_product product ON product.id = masterorderproduct.product_id LEFT JOIN basic_company company ON company.id = masterorder.company_id LEFT JOIN technologies_technology technology ON technology.id = masterorderproduct.technology_id LEFT JOIN orders_order orders ON orders.masterorder_id = masterorderproduct.masterorder_id AND orders.product_id = masterorderproduct.product_id LEFT JOIN basic_company companypayer ON companypayer.id = masterorder.companypayer_id LEFT JOIN basic_assortment assortment ON assortment.id = product.assortment_id WHERE masterorder.masterordertype = '03manyProducts' AND masterorderproduct.id IS NOT NULL GROUP BY masterorderdefinition.number, masterorder.id, masterorder.product_id, masterorderproduct.id, masterorder.masterordertype, masterorder.name, masterorder.deadline, masterorder.masterorderstate, masterorder.masterorderpositionstatus, masterorder.comments, product.number, product.name, product.unit, technology.number, company.name, masterorder.active, companypayer.name, assortment.name;
...
CREATE OR REPLACE VIEW productioncounting_performanceanalysisdto_helper AS SELECT (100 * SUM(performanceanalysisdetaildto.timebasedonnorms)::numeric / (8 * 60 * 60))::numeric(14,2) AS dailyperformance, COUNT(DISTINCT performanceanalysisdetaildto.productionline_id) AS productionlinequantity, staff_id, timerangefromwithouttime FROM productioncounting_performanceanalysisdetaildto performanceanalysisdetaildto GROUP BY staff_id, timerangefromwithouttime;

CREATE OR REPLACE VIEW productioncounting_performanceanalysisdto AS SELECT row_number() OVER () AS id, BOOL_OR(performanceanalysisdetaildto.active) AS active, performanceanalysisdetaildto.productionline_id::integer AS productionline_id, performanceanalysisdetaildto.productionlinenumber AS productionlinenumber, performanceanalysisdetaildto.staff_id::integer AS staff_id, performanceanalysisdetaildto.staffname AS staffname, COALESCE(SUM(performanceanalysisdetaildto.timebasedonnorms)::integer, 0) AS timebasedonnormssum, (8 * 60 * 60)::integer AS labortimesum, (COALESCE(SUM(performanceanalysisdetaildto.timebasedonnorms)::integer, 0) - (8 * 60 * 60))::integer AS timedeviation, (100 * SUM(performanceanalysisdetaildto.timebasedonnorms)::numeric / (8 * 60 * 60))::numeric(14,2) AS performance, performanceanalysisdetaildto.shift_id::integer AS shift_id, performanceanalysisdetaildto.shiftname, performanceanalysisdetaildto.timerangefromwithouttime AS timerangefrom, performanceanalysisdetaildto.timerangetowithouttime AS timerangeto, SUM(performanceanalysisdetaildto.donequantity) AS donequantity, MIN(helper.productionlinequantity) AS productionlinequantity, COUNT(DISTINCT performanceanalysisdetaildto.assortment_id) AS assortmentquantity, MIN(helper.dailyperformance)::numeric(14,2) AS dailyperformance FROM productioncounting_performanceanalysisdetaildto performanceanalysisdetaildto LEFT JOIN productioncounting_performanceanalysisdto_helper helper ON performanceanalysisdetaildto.staff_id = helper.staff_id AND performanceanalysisdetaildto.timerangefromwithouttime = helper.timerangefromwithouttime GROUP BY productionline_id, productionlinenumber, performanceanalysisdetaildto.staff_id, staffname, shift_id, shiftname, performanceanalysisdetaildto.timerangefromwithouttime, timerangetowithouttime;
...
-- VIEW: ordersgroups_plannedworkingtimeanalysisdto

DROP TABLE IF EXISTS ordersgroups_plannedworkingtimeanalysisdto;

CREATE OR REPLACE VIEW ordersgroups_plannedworkingtimeanalysisdto AS SELECT DISTINCT ordersgroup.id, ordersgroup.active, company.number AS company, productionline.number AS productionlinenumber, assortment.name AS assortmentname, ordersgroup.number, ordersgroup.quantity, FIRST_VALUE(product.unit) OVER (PARTITION BY ordersgroup.id ORDER BY ordersorder.id) AS unit, FIRST_VALUE(technology.standardperformancetechnology) OVER (PARTITION BY ordersgroup.id ORDER BY ordersorder.id) AS performancenorm, (ordersgroup.quantity::numeric * 60 / (FIRST_VALUE(technology.standardperformancetechnology) OVER (PARTITION BY ordersgroup.id ORDER BY ordersorder.id)))::integer AS timebasedonnorms, ((ordersgroup.quantity::numeric * 60 / (FIRST_VALUE(technology.standardperformancetechnology) OVER (PARTITION BY ordersgroup.id ORDER BY ordersorder.id))) / (8 * 60 * 60))::numeric(14,1) AS shiftquantity, ordersgroup.startdate, ordersgroup.finishdate, masterorder.deadline, FIRST_VALUE(tcontext.number) OVER (PARTITION BY ordersgroup.id ORDER BY ordersorder.id) AS generatorname FROM ordersgroups_ordersgroup ordersgroup JOIN basic_assortment assortment ON ordersgroup.assortment_id = assortment.id JOIN productionlines_productionline productionline ON ordersgroup.productionline_id = productionline.id JOIN masterorders_masterorder masterorder ON ordersgroup.masterorder_id = masterorder.id LEFT JOIN basic_company company ON company.id = masterorder.company_id LEFT JOIN orders_order ordersorder ON ordersorder.ordersgroup_id = ordersgroup.id LEFT JOIN basic_product product ON ordersorder.product_id = product.id LEFT JOIN technologiesgenerator_generatortechnologiesforproduct tgenn ON ordersorder.technologyprototype_id = tgenn.technology_id LEFT JOIN technologiesgenerator_generatorcontext tcontext ON tcontext.id = tgenn.generatorcontext_id LEFT JOIN technologies_technology technology ON technology.id = ordersorder.technology_id WHERE ordersgroup.state IN ('01draft', '02inProgress');
...
CREATE OR REPLACE VIEW materialflowresources_palletstoragestatedto AS SELECT ROW_NUMBER() OVER () AS id, TRUE AS active, palletstoragestatedetails.palletnumber, palletstoragestatedetails.palletnumberactive, palletstoragestatedetails.typeofpallet, palletstoragestatedetails.storagelocationnumber, palletstoragestatedetails.locationnumber, SUM(palletstoragestatedetails.quantity)::NUMERIC(14,5) AS totalquantity, NULL::bigint AS palletmovehelper_id, NULL::bigint AS newstoragelocation_id, NULL::bigint AS newpalletnumber_id FROM materialflowresources_palletstoragestatedetailsdto palletstoragestatedetails GROUP BY palletstoragestatedetails.palletnumber, palletstoragestatedetails.palletnumberactive, palletstoragestatedetails.typeofpallet, palletstoragestatedetails.storagelocationnumber, palletstoragestatedetails.locationnumber ORDER BY palletstoragestatedetails.palletnumber, palletstoragestatedetails.locationnumber;
-- end

CREATE SEQUENCE technologies_barcodeoperationcomponent_number_seq;
