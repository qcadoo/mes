...
CREATE OR REPLACE VIEW masterorders_masterorderpositiondto AS SELECT (SELECT COALESCE(max(masterorders_masterorderproduct.id), 0::numeric) FROM masterorders_masterorderproduct) + row_number() OVER () AS id, masterorderdefinition.name, masterorder.id::integer AS masterorderid, masterorder.product_id::integer AS productid, masterorderproduct.id::integer AS masterorderproductid, masterorder.masterordertype, masterorder.name AS masterordername, masterorder.number, masterorder.deadline, masterorder.masterorderstate AS masterorderstatus, masterorder.masterorderpositionstatus, COALESCE(masterorder.masterorderquantity, 0::numeric) AS masterorderquantity, COALESCE((SELECT SUM(orders.plannedquantity) FROM orders_order orders WHERE orders.masterorder_id = masterorder.id AND orders.product_id = masterorder.product_id), 0::numeric) AS cumulatedmasterorderquantity, COALESCE((SELECT SUM(orders.donequantity) FROM orders_order orders WHERE orders.masterorder_id = masterorder.id AND orders.product_id = masterorder.product_id), 0::numeric) AS producedorderquantity, CASE WHEN (masterorder.masterorderquantity - COALESCE((SELECT SUM(orders.donequantity) FROM orders_order orders WHERE orders.masterorder_id = masterorder.id AND orders.product_id = masterorder.product_id), 0::numeric)) > 0 THEN (masterorder.masterorderquantity - COALESCE((SELECT SUM(orders.donequantity) FROM orders_order orders WHERE orders.masterorder_id = masterorder.id AND orders.product_id = masterorder.product_id), 0::numeric)) ELSE 0::numeric END AS lefttorelease, masterorder.comments, product.number AS productnumber, product.name AS productname, product.unit, technology.number AS technologyname, company.name AS companyname, masterorder.active FROM masterorders_masterorder masterorder LEFT JOIN masterorders_masterorderdefinition masterorderdefinition ON masterorderdefinition.id = masterorder.masterorderdefinition_id LEFT JOIN masterorders_masterorderproduct masterorderproduct ON masterorderproduct.masterorder_id = masterorder.id LEFT JOIN basic_product product ON product.id = masterorder.product_id LEFT JOIN basic_company company ON company.id = masterorder.company_id LEFT JOIN technologies_technology technology ON technology.id = masterorder.technology_id WHERE masterorder.masterordertype::text = '02oneProduct'::text UNION ALL SELECT COALESCE(masterorderproduct.id, 0::numeric), masterorderdefinition.name, masterorder.id::integer AS masterorderid, masterorderproduct.product_id::integer AS productid, masterorderproduct.id::integer AS masterorderproductid, masterorder.masterordertype, masterorder.name AS masterordername, masterorder.number, masterorder.deadline, masterorder.masterorderstate AS masterorderstatus, masterorderproduct.masterorderpositionstatus, COALESCE(masterorderproduct.masterorderquantity, 0::numeric) AS masterorderquantity, COALESCE((SELECT SUM(orders.plannedquantity) FROM orders_order orders WHERE orders.masterorder_id = masterorderproduct.masterorder_id AND orders.masterorderproductcomponent_id = masterorderproduct.id AND orders.product_id = masterorderproduct.product_id), 0::numeric) AS cumulatedmasterorderquantity, COALESCE((SELECT SUM(orders.donequantity) FROM orders_order orders WHERE orders.masterorder_id = masterorderproduct.masterorder_id AND orders.masterorderproductcomponent_id = masterorderproduct.id AND orders.product_id = masterorderproduct.product_id), 0::numeric) AS producedorderquantity, CASE WHEN (masterorderproduct.masterorderquantity - COALESCE((SELECT SUM(orders.donequantity) FROM orders_order orders WHERE orders.masterorder_id = masterorderproduct.masterorder_id AND orders.masterorderproductcomponent_id = masterorderproduct.id AND orders.product_id = masterorderproduct.product_id), 0::numeric)) > 0 THEN (masterorderproduct.masterorderquantity - COALESCE((SELECT SUM(orders.donequantity) FROM orders_order orders WHERE orders.masterorder_id = masterorderproduct.masterorder_id AND orders.masterorderproductcomponent_id = masterorderproduct.id AND orders.product_id = masterorderproduct.product_id), 0::numeric)) ELSE 0::numeric END AS lefttorelease, masterorderproduct.comments, product.number AS productnumber, product.name AS productname, product.unit, technology.number AS technologyname, company.name AS companyname, masterorder.active FROM masterorders_masterorder masterorder LEFT JOIN masterorders_masterorderdefinition masterorderdefinition ON masterorderdefinition.id = masterorder.masterorderdefinition_id LEFT JOIN masterorders_masterorderproduct masterorderproduct ON masterorderproduct.masterorder_id = masterorder.id LEFT JOIN basic_product product ON product.id = masterorderproduct.product_id LEFT JOIN basic_company company ON company.id = masterorder.company_id LEFT JOIN technologies_technology technology ON technology.id = masterorderproduct.technology_id WHERE masterorder.masterordertype = '03manyProducts' AND masterorderproduct.id IS NOT NULL;
