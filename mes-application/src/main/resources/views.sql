...
CREATE OR REPLACE VIEW productioncounting_performanceanalysisdto AS SELECT row_number() OVER () AS id, BOOL_OR(performanceanalysisdetaildto.active) AS active, performanceanalysisdetaildto.productionline_id::integer AS productionline_id, performanceanalysisdetaildto.productionlinenumber AS productionlinenumber, performanceanalysisdetaildto.staff_id::integer AS staff_id, performanceanalysisdetaildto.staffname AS staffname, SUM(performanceanalysisdetaildto.timebasedonnorms)::integer AS timebasedonnormssum, (8 * 60 * 60)::integer AS labortimesum, (SUM(performanceanalysisdetaildto.timebasedonnorms) - (8 * 60 * 60))::integer AS timedeviation, ((SUM(performanceanalysisdetaildto.timebasedonnorms) * 100)::numeric(14,5) / (8 * 60 * 60))::numeric(14,5) AS performance, performanceanalysisdetaildto.shift_id::integer AS shift_id, performanceanalysisdetaildto.shiftname AS shiftname, MIN(performanceanalysisdetaildto.timerangefrom) AS timerangefrom, MAX(performanceanalysisdetaildto.timerangeto) AS timerangeto FROM productioncounting_performanceanalysisdetaildto performanceanalysisdetaildto GROUP BY productionline_id, productionlinenumber, staff_id, staffname, shift_id, shiftname;
