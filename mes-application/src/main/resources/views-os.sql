...
CREATE OR REPLACE VIEW public.masterorders_masterorderposition_manyproducts AS SELECT COALESCE(masterorderproduct.id, 0::bigint) AS "coalesce", masterorderdefinition.name, masterorder.id::integer AS masterorderid, masterorderproduct.product_id::integer AS productid, masterorderproduct.id::integer AS masterorderproductid, masterorder.masterordertype, masterorder.name AS masterordername, masterorder.number, masterorder.deadline, masterorder.masterorderstate AS masterorderstatus, masterorderproduct.masterorderpositionstatus, COALESCE(masterorderproduct.masterorderquantity, 0::numeric) AS masterorderquantity, COALESCE(( SELECT sum(orders.plannedquantity) AS sum), 0::numeric) AS cumulatedmasterorderquantity, COALESCE(( SELECT sum(orders.donequantity) AS sum), 0::numeric) AS producedorderquantity, CASE WHEN (COALESCE(masterorderproduct.masterorderquantity, 0::numeric) - COALESCE(( SELECT sum(orders.donequantity) AS sum), 0::numeric)) > 0::numeric THEN COALESCE(masterorderproduct.masterorderquantity, 0::numeric) - COALESCE(( SELECT sum(orders.donequantity) AS sum), 0::numeric) ELSE 0::numeric END AS lefttorelease, masterorderproduct.comments, product.number AS productnumber, product.name AS productname, product.unit, technology.number AS technologyname, company.name AS companyname, masterorder.active, companypayer.name AS companypayer FROM masterorders_masterorder masterorder LEFT JOIN masterorders_masterorderdefinition masterorderdefinition ON masterorderdefinition.id = masterorder.masterorderdefinition_id LEFT JOIN masterorders_masterorderproduct masterorderproduct ON masterorderproduct.masterorder_id = masterorder.id LEFT JOIN basic_product product ON product.id = masterorderproduct.product_id LEFT JOIN basic_company company ON company.id = masterorder.company_id LEFT JOIN technologies_technology technology ON technology.id = masterorderproduct.technology_id LEFT JOIN orders_order orders ON orders.masterorderproductcomponent_id = masterorderproduct.id AND orders.masterorder_id = masterorderproduct.masterorder_id AND orders.product_id = masterorderproduct.product_id LEFT JOIN basic_company companypayer ON companypayer.id = masterorder.companypayer_id WHERE masterorder.masterordertype::text = '03manyProducts'::text AND masterorderproduct.id IS NOT NULL GROUP BY companypayer.name,masterorderdefinition.name, masterorder.id, masterorder.product_id, masterorderproduct.id, masterorder.masterordertype, masterorder.name, masterorder.deadline, masterorder.masterorderstate, masterorder.masterorderpositionstatus, masterorder.comments, product.number, product.name, product.unit, technology.number, company.name, masterorder.active;
...
CREATE OR REPLACE VIEW materialflowresources_palletstoragestatedto AS SELECT DISTINCT ROW_NUMBER() OVER () AS id, TRUE AS active, palletnumber.id::INTEGER AS palletnumber_id, palletnumber.number AS palletnumber, resource.typeofpallet AS typeofpallet, storagelocation.id::INTEGER AS storagelocation_id, storagelocation.number AS storagelocationnumber, location.id::INTEGER AS location_id, location.number AS locationnumber FROM materialflowresources_resource resource LEFT JOIN basic_palletnumber palletnumber ON palletnumber.id = resource.palletnumber_id LEFT JOIN materialflowresources_storagelocation storagelocation ON storagelocation.id = resource.storagelocation_id LEFT JOIN materialflow_location location ON location.id = resource.location_id WHERE resource.palletnumber_id IS NOT NULL ORDER BY palletnumber.number;
